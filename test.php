...
if (isset($_GET['download_file'])) {
    ...
    if (!is_file($filePath) || basename(realpath($filePath)) !== $fileName || strpos(realpath($filePath), realpath($fullPath)) !== 0) {
...
<body>
    <header>
        <div class="breadcrumb">
            ...
            $isRootProjectsDirectory = (trim($currentPathDecoded, '/') === 'projects/');
            ...
            if (!is_dir($fullCurrentDir)) {
                ...
                if ($files === false) {
                    ...
                    foreach ($actualFiles as $file) {
                        ...
                        $filePath = $fullCurrentDir . '/' . $file;
                        $fileRelativePathFromDocRoot = str_replace(realpath($_SERVER['DOCUMENT_ROOT']) . '/', '', realpath($filePath));
                        ...
                        if ($fileType) {
                            $firstFileToPreview = ['path' => $fileRelativePathFromDocRoot, 'type' => $fileType];
                        }

                        if ($isRootProjectsDirectory && in_array(strtolower($extension), $imageExtensions)) {
                            $phpHomeImagesForSlideshow[] = $fileRelativePathFromDocRoot;
                        }
                    }
                    ...
                    if ($firstFileToPreview === null && !empty($directories)) {
                        ...
                        $firstFileToPreview = ['path' => str_replace(realpath($_SERVER['DOCUMENT_ROOT']) . '/', '', realpath($firstDirectoryPath)), 'type' => 'directory'];
                    }
                    ...
                    foreach ($allDisplayItems as $file) {
                        ...
                        $filePath = $fullCurrentDir . '/' . $file;
                        $itemRelativePathFromDocRoot = str_replace(realpath($_SERVER['DOCUMENT_ROOT']) . '/', '', realpath($filePath));
                        $extension = pathinfo($file, PATHINFO_EXTENSION);
                        ...
                        if (is_dir($filePath)) {
                            ...
                            if ($fileType) {
                                ...
                                echo "<a href='#' class='previewable-file-link{$hideClass}' data-path='" . htmlspecialchars($itemRelativePathFromDocRoot) . "' data-type='" . htmlspecialchars($fileType) . "'>" . $fileLowercase . "</a>";
                            } else {
